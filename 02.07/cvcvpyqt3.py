...
class Orim(QMainWindow):
    ...
    def fileOpenFunction(self):
        ...
        self.mask[:,:]=cv2.GC_PR_BGD	# 모든 화소를 배경일 것 같음으로 초기화
    ...
    def painting(self,event,x,y,flags,param):
        if event==cv2.EVENT_LBUTTONDOWN:   
            cv2.circle(self.img_show,(x,y),self.BrushSiz,self.LColor,-1) # 왼쪽 버튼을 클릭하면 파란색
            cv2.circle(self.mask,(x,y),self.BrushSiz,cv2.GC_FGD,-1)
        elif event==cv2.EVENT_RBUTTONDOWN: 
            cv2.circle(self.img_show,(x,y),self.BrushSiz,self.RColor,-1) # 오른쪽 버튼을 클릭하면 빨간색
            cv2.circle(self.mask,(x,y),self.BrushSiz,cv2.GC_BGD,-1)
        elif event==cv2.EVENT_MOUSEMOVE and flags==cv2.EVENT_FLAG_LBUTTON:
            cv2.circle(self.img_show,(x,y),self.BrushSiz,self.LColor,-1) # 왼쪽 버튼을 클릭하고 이동하면 파란색
            cv2.circle(self.mask,(x,y),self.BrushSiz,cv2.GC_FGD,-1)
        elif event==cv2.EVENT_MOUSEMOVE and flags==cv2.EVENT_FLAG_RBUTTON:
            cv2.circle(self.img_show,(x,y),self.BrushSiz,self.RColor,-1) # 오른쪽 버튼을 클릭하고 이동하면 빨간색 
            cv2.circle(self.mask,(x,y),self.BrushSiz,cv2.GC_BGD,-1)
